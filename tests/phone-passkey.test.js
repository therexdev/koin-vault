...
(async () => {
  ...
  const secondCreation = requests.at(-1);
  assert.equal(secondCreation.excludeCredentials.length, 0, 'An existing wallet must not block a separate wallet on the same authenticator');
  assert.notDeepEqual(secondCreation.user.id, requests[0].user.id, 'Separate wallets use separate user handles');
  assert.notEqual(secondCreation.user.name, requests[0].user.name, 'New wallet passkeys have distinguishable picker labels');
  assert.equal(secondCreation.user.name, secondCreation.user.displayName);
  ...
  const primaryId = passkey.storedId();
  await passkey.createBackupCredential([created.credentialId]);
  assert.equal(requests.at(-1).excludeCredentials.length, 1, 'Backups still exclude credentials already registered to that wallet');
  assert.deepEqual(Buffer.from(requests.at(-1).excludeCredentials[0].id), Buffer.from(rawId));
  assert.equal(passkey.storedId(), primaryId, 'Creating a backup does not replace the active credential');
