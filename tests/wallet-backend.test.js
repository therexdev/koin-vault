...
(async () => {
  ...
  const upstream = http.createServer(async (req, res) => {
    ...
    if (req.url.endsWith('/api/config')) return res.end(JSON.stringify({ ok: true, demo: false, rpId: 'wallet.usekoinos.com', maxCredentialsPerAccount: 32, sendAssets: ['koin', 'vhp'], features: { buy: !req.url.startsWith('/android') } }));
  ...
  try {
    for (const prefix of ['', '/android']) {
      ...
      assert.deepEqual(value.sendAssets, ['koin', 'vhp'], 'Forward the backend send capabilities unchanged');
